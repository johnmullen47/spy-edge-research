"""Research-only sector-context event outcome studies."""
...
SECTOR_EVENT_STUDY_CAVEAT = "sector_event_study_is_descriptive_research_only"
...
def summarize_event_by_sector_context(
    df: pd.DataFrame,
    event_column: str,
    outcome_column: str,
    ...
    *,
    ...
    """Summarize one event/outcome pair inside sector context groups."""
        ...
        rows.append(
            {
                ...
                **summary.to_dict(),
                "study_caveat": SECTOR_EVENT_STUDY_CAVEAT,
            }
        )
    return pd.DataFrame(rows, columns=_study_columns(contexts))


def compare_sector_confirmed_event_outcomes(
    summary_table: pd.DataFrame,
    *,
    context_column: str = "primary_sector_context",
    context_values: Mapping[str, str] | None = None,
) -> pd.DataFrame:
    """Compare descriptive event outcomes across named sector contexts."""
    labels = dict(
        context_values
        or {
            "sector_confirmed": "sector_confirmed",
            "sector_divergent": "sector_divergent",
            "defensive_led": "defensive_led",
            "cyclical_led": "cyclical_led",
    ...
    for label, value in labels.items():
        subset = summary_table.loc[summary_table[context_column].eq(value), [*keys, "event_count", "event_expectancy", "event_hit_rate"]]
    ...
    base["comparison_caveat"] = "descriptive_sector_context_comparison_not_edge_claim"
    return base


def build_sector_event_outcome_table(
    df: pd.DataFrame,
    ...
    *,
    ...
    """Build sector-context conditioned outcome rows for each catalog event."""
...
def summarize_sector_context_coverage(
    df: pd.DataFrame,
    context_columns: Iterable[str],
    *,
    min_context_rows: int = 1,
) -> pd.DataFrame:
    """Summarize sector context availability and sample sizes."""
    contexts = _normalize_columns(context_columns, "context_columns")
    _validate_positive_int(min_context_rows, "min_context_rows")
    ...
    rows.append(
        {
            "context_key": "__missing_sector_context_rows__",
    ...
    return pd.DataFrame(rows, columns=[*SECTOR_CONTEXT_COLUMNS, *contexts])


def build_sector_event_research_report(
    df: pd.DataFrame,
    catalog: pd.DataFrame,
    outcome_columns: Iterable[str],
    context_columns: Iterable[str],
    *,
    ...
    """Build deterministic sector-event report tables for research review."""
    contexts = _normalize_columns(context_columns, "context_columns")
    outcomes = _normalize_columns(outcome_columns, "outcome_columns")
    outcome_table = build_sector_event_outcome_table(
        df,
        catalog,
        outcomes,
        contexts,
    ...
    comparison_table = (
        compare_sector_confirmed_event_outcomes(outcome_table, context_column=comparison_column)
        if not outcome_table.empty and comparison_column in outcome_table.columns
        else pd.DataFrame()
    )
    return {
        "metadata": {
            "report_caveat": SECTOR_EVENT_STUDY_CAVEAT,
        ...
        "context_coverage": summarize_sector_context_coverage(
            df,
            contexts,
            min_context_rows=min_context_rows,
        ),
        "event_outcomes": outcome_table,
        "sector_context_comparison": comparison_table,
    }
